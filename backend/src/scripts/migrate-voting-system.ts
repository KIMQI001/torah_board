...
async function migrateVotingSystem() {
  try {
    ...
    for (const dao of daos) {
      ...
      const updatedMembers = await prisma.dAOMember.findMany({
        where: { daoId: dao.id, status: 'ACTIVE' }
      });

      for (const member of updatedMembers) {
        try {
          await VotingWeightService.calculateVotingPower(member.id);
        } catch (error) {
      ...
      // 步骤5：验证权重分配
      const finalMembers = await prisma.dAOMember.findMany({
        where: { daoId: dao.id, status: 'ACTIVE' }
      });
      ...
      for (const member of finalMembers) {
        ...
        if (member.role === 'CHAIR') chairWeight += member.votingPower;
        else if (member.role === 'ADMIN') adminWeight += member.votingPower;
        else if (member.role === 'MEMBER') memberWeight += member.votingPower;
      }
    ...
    // 步骤6：更新默认提案阈值
    console.log('\n🎯 更新提案默认阈值为31%...');
    await prisma.dAOProposal.updateMany({
      where: {
...
async function generateMigrationReport() {
  const totalDAOs = await prisma.dAO.count();
  const totalMembers = await prisma.dAOMember.count({ where: { status: 'ACTIVE' } });
  
  const roleDistribution = await prisma.dAOMember.groupBy({
    by: ['role'],
    where: { status: 'ACTIVE' },
    _count: { role: true }
  });

  const avgVotingPower = await prisma.dAOMember.aggregate({
    where: { status: 'ACTIVE' },
