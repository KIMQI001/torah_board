...
export class DashboardService {
  ...
  static async getDashboardStats(userId: string): Promise<DashboardStats> {
    try {
      Logger.info('Generating dashboard stats', { userId });
   ...
   * Calculate overview statistics
   */
  private static calculateOverviewStats(nodes: any[]): {
    totalNodes: number;
    onlineNodes: number;
    totalCapacity: string;
    totalEarnings: number;
   ...
   */
  private static async getRecentActivity(userId: string): Promise<Array<{
    type: 'node_added' | 'capacity_updated' | 'performance_recorded' | 'earnings_updated';
    message: string;
    timestamp: string;
    nodeId?: string;
    ...
    try {
      ...
      const recentNodes = await prisma.userNode.findMany({
        where: {
          userId,
          createdAt: {
      ...
      const recentCapacityUpdates = await prisma.userNode.findMany({
        where: {
          userId,
          capacity: { not: null },
          updatedAt: {
      ...
      recentCapacityUpdates.forEach(node => {
        if (node.capacity && node.capacity !== 'Querying...') {
          activities.push({
            type: 'capacity_updated' as const,
      ...
      const recentPerformance = await prisma.nodePerformance.findMany({
        where: {
          node: { userId }
        },
        include: {
      ...
      recentPerformance.forEach(perf => {
        activities.push({
          type: 'performance_recorded' as const,
   ...
   * Calculate nodes by status distribution
   */
  private static calculateNodesByStatus(nodes: any[]): Array<{
    status: string;
    count: number;
    percentage: number;
  }> {
   ...
   * Calculate nodes by project distribution
   */
  private static calculateNodesByProject(nodes: any[]): Array<{
    projectName: string;
    category: string;
    nodeCount: number;
    totalCapacity: string;
    ...
    const projectGroups = nodes.reduce((groups, node) => {
      ...
      return groups;
    }, {} as Record<string, { category: string; nodes: any[] }>);
   ...
   */
  private static async generateEarningsChart(userId: string): Promise<Array<{
    date: string;
    earnings: number;
    cumulativeEarnings: number;
  }>> {
    try {
      ...
      Logger.error('Error generating earnings chart', {
        error: error.message,
        userId
      });
      return [];
   ...
   */
  private static async calculatePerformanceMetrics(userId: string): Promise<{
    averageCpuUsage: number;
    averageMemoryUsage: number;
    averageDiskUsage: number;
    averageNetworkLatency: number;
    ...
    try {
      ...
      const performanceData = await prisma.nodePerformance.findMany({
        where: {
          node: { userId },
          timestamp: {
   ...
   */
  private static getTopPerformingNodes(nodes: any[]): Array<{
    nodeId: string;
    projectName: string;
    capacity: string;
    uptime: string;
    ...
    return nodes
      .map(node => {
        ...
        else if (node.status === 'SYNCING') score += 10;
