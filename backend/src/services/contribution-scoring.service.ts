...
export class ContributionScoringService {
  private static readonly DEFAULT_WEIGHTS: ContributionWeights = {
   ...
   * 计算成员的贡献分数
   */
  static async calculateContributionScore(
    memberId: string,
    customWeights?: Partial<ContributionWeights>
  ): Promise<ContributionScoreBreakdown> {
    try {
      ...
      // 计算各项分数
      const proposalScore = 
        metrics.proposalsCreated * weights.proposalCreation +
      ...
      const consistencyScore = this.calculateConsistencyScore(
        metrics.membershipDuration,
        metrics.lastActivityDays,
        weights.consistency
      );
      ...
      Logger.info('Contribution score calculated', {
        memberId,
        totalScore,
        breakdown: {
          proposalScore,
      ...
      Logger.error('Failed to calculate contribution score', { error, memberId });
      throw error;
    }
  }
   ...
   * 获取贡献指标
   */
  private static async getContributionMetrics(member: any): Promise<ContributionMetrics> {
    const now = new Date();
    ...
    const approvedProposals = proposals.filter(p => 
      ['EXECUTED', 'PASSED'].includes(p.status)
    ).length;
    ...
    // 获取任务完成指标
    const completedTasks = await prisma.dAOTask.count({
      where: {
   ...
   * 获取项目贡献数
   */
  private static async getProjectContributions(memberId: string): Promise<number> {
    ...
    const createdProjects = await prisma.dAOProject.count({
      where: { createdBy: member.userId }
    });

    // 查找作为团队成员参与的项目
    const participatedProjects = await prisma.dAOProject.count({
      where: {
        daoId: member.daoId,
   ...
   * 获取里程碑交付数
   */
  private static async getMilestoneDeliveries(memberId: string): Promise<number> {
    const verifications = await prisma.dAOMilestoneVerification.count({
      where: {
   ...
   * 计算奖励分数
   */
  private static calculateBonusScore(metrics: ContributionMetrics): number {
    let bonus = 0;
   ...
   * 批量更新DAO所有成员的贡献分数
   */
  static async updateDAOContributionScores(daoId: string): Promise<void> {
    try {
      ...
      Logger.info('DAO contribution scores updated', {
        daoId,
        memberCount: members.length
      });
    } catch (error) {
      Logger.error('Failed to update DAO contribution scores', { error, daoId });
      throw error;
    }
  }
   ...
   * 获取贡献分数排行榜
   */
  static async getContributionLeaderboard(
    daoId: string,
    limit: number = 10
  ): Promise<Array<{
    memberId: string;
    ...
    try {
      ...
      Logger.error('Failed to get contribution leaderboard', { error });
      throw error;
    }
  }
   ...
   * 格式化最后活动时间
   */
  private static formatLastActivity(lastActivity: Date): string {
    const now = new Date();
   ...
   * 基于贡献分数调整声誉值
   */
  static async updateReputationBasedOnContribution(memberId: string): Promise<void> {
    try {
      const member = await prisma.dAOMember.findUnique({
        where: { id: memberId }
      });
      ...
      let newReputation = Math.min(
        50 + (member.contributionScore / 100) * 10,
        100
      );
      ...
      Logger.info('Reputation updated based on contribution', {
        memberId,
        contributionScore: member.contributionScore,
        newReputation
      });
    } catch (error) {
      Logger.error('Failed to update reputation based on contribution', { error });
    }
  }
   ...
   * 获取成员贡献历史
   */
  static async getMemberContributionHistory(
    memberId: string,
    months: number = 6
  ): Promise<Array<{
    month: string;
    ...
    try {
      ...
      Logger.error('Failed to get member contribution history', { error });
      throw error;
    }
  }
}