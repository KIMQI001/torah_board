...
import { AuthenticatedRequest, CreateProjectRequest, UpdateProjectRequest, QueryOptions } from '@/types';
...
export class ProjectsController {
  ...
  static async getProjects(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (blockchain) {
        where.blockchain = {
          contains: blockchain as string,
          mode: 'insensitive'
        };
      }

      // Get total count for pagination
      const total = await prisma.dePINProject.count({ where });
  ...
  static async getProject(req: Request, res: Response): Promise<void> {
    try {
      ...
      ResponseUtil.success(res, transformedProject, 'Project retrieved successfully');
  ...
  static async createProject(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const projectData = req.body as CreateProjectRequest;

      // Check if project name already exists
      const existingProject = await prisma.dePINProject.findUnique({
        where: { name: projectData.name }
      });

      if (existingProject) {
        ResponseUtil.error(res, 'Project with this name already exists');
        return;
      }

      // Create new project
      const project = await prisma.dePINProject.create({
        data: {
      ...
      Logger.info('New DePIN project created', {
      ...
      ResponseUtil.success(res, transformedProject, 'Project created successfully');
  ...
  static async updateProject(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      if (updates.name && updates.name !== existingProject.name) {
        const nameExists = await prisma.dePINProject.findUnique({
      ...
      // Update project
      const updatedProject = await prisma.dePINProject.update({
        where: { id },
        data: {
          ...updates,
          updatedAt: new Date()
      ...
      Logger.info('DePIN project updated', {
        projectId: id,
      ...
      ResponseUtil.success(res, transformedProject, 'Project updated successfully');
  ...
  static async deleteProject(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      if (project._count.nodes > 0) {
        ResponseUtil.error(res, 'Cannot delete project with active nodes');
        return;
      }

      // Delete project
      await prisma.dePINProject.delete({
        where: { id }
      });

      Logger.info('DePIN project deleted', {
        projectId: id,
  ...
  static async getProjectsByCategory(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!validCategories.includes(upperCategory)) {
        ResponseUtil.error(res, 'Invalid category');
        return;
      }

      const where = {
        category: upperCategory as any,
        status: 'ACTIVE'
      };
      ...
      Logger.error('Error getting projects by category', { error: error.message });
      ResponseUtil.serverError(res);
    }
  }
}