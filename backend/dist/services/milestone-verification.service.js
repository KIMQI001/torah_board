...
exports.MilestoneVerificationService = void 0;
...
class MilestoneVerificationService {
     ...
     * 提交里程碑验证申请
     */
    static async submitMilestoneForVerification(milestoneId, evidenceSubmission, submittedBy) {
        try {
            // 获取里程碑信息
            const milestone = await database_1.prisma.dAOMilestone.findUnique({
            ...
            // 检查提交者权限
            const submitter = await database_1.prisma.dAOMember.findFirst({
                where: {
            ...
            // 创建验证记录
            const verification = await database_1.prisma.dAOMilestoneVerification.create({
                data: {
                    milestoneId,
            ...
            // 更新里程碑状态
            await database_1.prisma.dAOMilestone.update({
                where: { id: milestoneId },
                data: { status: 'PENDING_VERIFICATION' }
            });
            logger_1.Logger.info('Milestone verification submitted', {
            ...
            logger_1.Logger.error('Failed to submit milestone for verification', { error });
            throw error;
        }
    }
    /**
     * 提交验证评审
     */
    static async submitVerificationReview(verificationId, reviewerId, review) {
        try {
            // 获取验证信息
            const verification = await database_1.prisma.dAOMilestoneVerification.findUnique({
            ...
            // 检查是否已经评审过
            const existingReview = await database_1.prisma.dAOVerificationReview.findUnique({
                where: {
            ...
            // 创建评审记录
            await database_1.prisma.dAOVerificationReview.create({
                data: {
                    verificationId,
                    reviewerId,
            ...
            // 更新验证状态
            await database_1.prisma.dAOMilestoneVerification.update({
                where: { id: verificationId },
                data: { status: 'UNDER_REVIEW' }
            });
            // 检查验证状态
            await this.checkVerificationStatus(verificationId);
            logger_1.Logger.info('Verification review submitted', {
                verificationId,
                reviewerId,
                decision: review.decision
            });
            ...
            logger_1.Logger.error('Failed to submit verification review', { error });
            throw error;
        }
    }
    /**
     * 检查验证状态
     */
    static async checkVerificationStatus(verificationId) {
        try {
            const verification = await database_1.prisma.dAOMilestoneVerification.findUnique({
                where: { id: verificationId },
                include: {
     ...
     * 验证里程碑
     */
    static async verifyMilestone(verificationId) {
        try {
            const verification = await database_1.prisma.dAOMilestoneVerification.findUnique({
                where: { id: verificationId },
                include: {
            ...
            logger_1.Logger.info('Milestone verified', { verificationId });
        }
        catch (error) {
            logger_1.Logger.error('Failed to verify milestone', { error });
        }
    }
    /**
     * 拒绝验证
     */
    static async rejectVerification(verificationId, reason) {
        try {
            await database_1.prisma.dAOMilestoneVerification.update({
                where: { id: verificationId },
                data: {
            ...
            logger_1.Logger.info('Milestone verification rejected', { verificationId, reason });
        }
        catch (error) {
            logger_1.Logger.error('Failed to reject verification', { error });
        }
    }
    /**
     * 处理里程碑付款
     */
    static async processMilestonePayment(verificationId) {
        try {
            const verification = await database_1.prisma.dAOMilestoneVerification.findUnique({
                where: { id: verificationId },
                include: {
            ...
            if (!verification)
                return;
            // 通过财务服务处理付款
            const transactionId = await treasury_service_1.TreasuryService.processMilestonePayment(verification.daoId, verification.projectId, verification.milestoneId, verification.milestone.budget, verification.submittedBy // 假设提交者为收款人，实际应该从项目信息中获取
            );
            // 更新验证状态
            await database_1.prisma.dAOMilestoneVerification.update({
                where: { id: verificationId },
            ...
            logger_1.Logger.info('Milestone payment processed', {
                verificationId,
                transactionId
            });
        }
        catch (error) {
            logger_1.Logger.error('Failed to process milestone payment', { error });
        }
    }
    /**
     * 获取验证者的总投票权
     */
    static async getTotalVerifierVotingPower(verificationId) {
        const verification = await database_1.prisma.dAOMilestoneVerification.findUnique({
            where: { id: verificationId }
        });
        if (!verification)
     ...
     * 确定验证方法
     */
    static determineVerificationMethod(milestone) {
        // 根据里程碑预算和类型确定验证方法
     ...
     * 选择验证者
     */
    static async selectVerifiers(daoId, budget) {
        // 根据预算选择不同的验证者
     ...
     * 获取里程碑验证列表
     */
    static async getMilestoneVerifications(daoId, status, limit = 20, offset = 0) {
        try {
            const where = { daoId };
            if (status)
                where.status = status;
            const verifications = await database_1.prisma.dAOMilestoneVerification.findMany({
                where,
                include: {
                    milestone: {
                        select: {
            ...
            logger_1.Logger.error('Failed to get milestone verifications', { error });
            throw error;
        }
    }
    /**
     * 获取单个验证详情
     */
    static async getMilestoneVerification(verificationId) {
        try {
            const verification = await database_1.prisma.dAOMilestoneVerification.findUnique({
                where: { id: verificationId },
                include: {
            ...
            logger_1.Logger.error('Failed to get milestone verification', { error });
            throw error;
        }
    }
}
exports.MilestoneVerificationService = MilestoneVerificationService;
//# sourceMappingURL=milestone-verification.service.js.map